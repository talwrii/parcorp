...
search_parser.add_argument(
    "--exact", "-x", action="store_true", default=False, help="search for exact string"
)
search_parser.add_argument(
    "--count",
    "-c",
...
search_parser.add_argument(
    "--words", "-w", type=int, help="Return searches with this many words"
)
...
def main():
    ...
    if args.command == "loadtmx":
        ...
        if args.target or args.source:
            ...
            condition = " or ".join(
                ["{} MATCH ?".format(language_field)]
                * (1 + len(args.additional_terms or []))
            )
            terms = (term.decode("utf8"),) + tuple(
                x.decode("utf8") for x in args.additional_terms
            )
        else:
            condition = " or ".join(
                ["german MATCH ? or english MATCH ?"]
                * (1 + len(args.additional_terms or []))
            )
            terms = (term.decode("utf8"), term.decode("utf8")) + tuple(
                itertools.chain.from_iterable(
                    (x.decode("utf8"), x.decode("utf8")) for x in args.additional_terms
                )
            )

        sql = "select german, english from translation where {} order by length(german)".format(
            condition
        )
        ...
        if args.count:
            ...
            for index, (german, english) in enumerate(cursor.fetchall()):
                ...
                if args.words is not None:
                    if (
                        min(len(german.split(" ")), len(english.split(" ")))
                        < args.words
                    ):
                        continue
                print("", german, end="")
